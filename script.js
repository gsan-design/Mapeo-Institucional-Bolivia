...
function initDashboard(data) {
  ...
  btn.addEventListener('click', () => {
    ...
    chartWrap.style.height = expanded ? `${Math.max(760, subset.length * 46)}px` : '360px';
  });
}
...
function renderInstrumentChart(rows) {
  ...
  instrumentChart = new Chart(ctx, {
    ...
    options: {
      ...
      scales: { x: { beginAtZero: true, ticks: { precision: 0 }, grid: { color: '#cccccc' } }, y: { ticks: { font: { size: 12 }, callback: function(value) { const label = this.getLabelForValue(value); return wrapLabel(label, 34); } }, grid: { display: false } } },
